...
const cropperTarget = ref<'menu'|'category'|'variant'|'topping'|'toppingMaster'>('menu')
  ...
  try {
    ...
    if (profile) {
      ...
      try {
        ...
        if (res.data) toppingMasters.value = res.data
      } catch {}
...
const handleFileChange = async (event: Event, target: 'menu'|'category'|'variant'|'topping'|'toppingMaster'='menu', vIdx?: {g:number;o:number}, tIdx?: {g:number;o:number}, tmIdx?: number) => {
...
const onCropped = async (payload: { blob: Blob; url: string; file: File }) => {
  ...
  } else if (cropperTarget.value==='toppingMaster' && cropperToppingMasterIdx.value!==null) {
...
const handleAddMenu = async () => {
  ...
  try {
    ...
    success('Menu ditambahkan (COS auto-delete saat hapus)')
...
const handleEditMenu = async () => {
  ...
  try {
    ...
    success('Menu diperbarui, gambar lama COS terhapus jika ganti.')
...
const openCategoryModal = (cat?:any) => {
  if (cat) { categoryForm.value={ name:cat.name, image_url:cat.image_url||'', sort_order:cat.sort_order||0, is_active:cat.is_active }; categoryPreviewUrl.value=cat.image_url||''; (categoryForm as any).id=cat.id } else { categoryForm.value={ name:'', image_url:'', sort_order:0, is_active:true }; categoryPreviewUrl.value=''; }
...
const handleDeleteCategory = async (id:string) => {
  if (!confirm('Hapus kategori? Menu jadi tanpa kategori. Icon COS akan dihapus.')) return
  deleteCategoryId.value=id
  try { await merchantsStore.deleteCategory(id); success('Kategori dihapus'); categories.value=await merchantsStore.fetchCategories() as any } catch { error('Gagal hapus') } finally { deleteCategoryId.value=null }
}
...
const handleSaveToppingMaster = async () => {
  ...
  try {
    ...
    success('Topping master disimpan (shared)')
    showToppingMasterModal.value=false
    try { const { request } = useApi(); const res=await request<{ data:any[] }>('/merchant/toppings'); if (res.data) toppingMasters.value=res.data } catch {}
  } catch { error('Gagal simpan topping master') }
}
const handleDeleteToppingMaster = async (id:string) => {
  if (!confirm('Hapus topping master ini? Gambar COS akan dihapus.')) return
  try { const { request } = useApi(); await request(`/merchant/toppings/${id}`, { method:'DELETE' }); success('Topping master dihapus'); toppingMasters.value=toppingMasters.value.filter(t=>t.id!==id) } catch { error('Gagal hapus') }
}
...
<template>
  <div class="px-4 pb-24 space-y-4">
    ...
    <div v-else class="space-y-4">
      <div class="flex items-center gap-2.5 pt-1">
        ...
        <div class="flex-1 min-w-0">
          ...
          <p class="text-[10px] text-slate-400 font-medium leading-tight">Menu · Kategori · Topping Master shared</p>
        </div>
        <button class="shrink-0 h-9 px-3.5 rounded-xl text-xs font-black bg-primary text-white flex items-center gap-1 shadow-md" @click="openAddModal"><Plus class="w-4 h-4" /> Tambah Menu</button>
      </div>

      <div class="flex gap-2 bg-slate-100 p-1 rounded-2xl">
        <button class="flex-1 h-8 rounded-xl text-[11px] font-black flex items-center justify-center gap-1" :class="activeTab==='menu' ? 'bg-white text-slate-900 shadow-sm border' : 'text-slate-500'" @click="activeTab='menu'"><Utensils class="w-3.5 h-3.5" /> Menu</button>
        <button class="flex-1 h-8 rounded-xl text-[11px] font-black flex items-center justify-center gap-1" :class="activeTab==='category' ? 'bg-white text-slate-900 shadow-sm border' : 'text-slate-500'" @click="activeTab='category'"><Tag class="w-3.5 h-3.5" /> Kategori</button>
        <button class="flex-1 h-8 rounded-xl text-[11px] font-black flex items-center justify-center gap-1" :class="activeTab==='topping' ? 'bg-white text-slate-900 shadow-sm border' : 'text-slate-500'" @click="activeTab='topping'"><ChefHat class="w-3.5 h-3.5" /> Topping</button>
      </div>
      ...
      <div v-if="activeTab==='menu'" class="space-y-3">
        ...
        <div v-if="filteredMenus.length===0" class="py-10 px-6 text-center bg-white border border-slate-100 rounded-[24px] shadow-sm">
          <div class="w-14 h-14 mx-auto rounded-2xl bg-slate-50 border flex items-center justify-center mb-3"><Utensils class="w-7 h-7 text-slate-400" /></div>
          <p class="text-[13px] font-black">Belum Ada Menu</p>
          <p class="text-[11px] text-slate-500 mt-1 max-w-[240px] mx-auto leading-relaxed">Tambah menu dengan foto wajib crop 1:1, varian ±harga dengan foto, topping independent shared bisa dipakai banyak minuman.</p>
          <button class="mt-4 h-10 px-5 rounded-full text-xs font-black bg-primary text-white shadow-md" @click="openAddModal">+ Tambah Menu</button>
        </div>
        ...
        <div v-else class="space-y-3">
          <div v-for="menu in filteredMenus as any" :key="menu.id" class="bg-white border border-slate-100 rounded-3xl p-4 flex gap-4 items-center justify-between shadow-sm">
            ...
            <div class="flex flex-col items-end gap-2 pl-3.5 border-l">
              <button class="h-7 px-3 text-[10px] font-black rounded-xl border flex items-center gap-1" :class="menu.is_available ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-rose-50 text-rose-500 border-rose-100'" :disabled="togglingMenuId===menu.id" @click="toggleMenuAvailable(menu)">
                <RefreshCw v-if="togglingMenuId===menu.id" class="w-3 h-3 animate-spin" /><template v-else>{{ menu.is_available ? 'Tersedia' : 'Habis' }}</template>
              </button>
              <div class="flex gap-1.5">
                ...
                <button class="w-8 h-8 rounded-xl border border-rose-100 flex items-center justify-center text-rose-500" :disabled="deletingMenuId===menu.id" @click="handleDeleteMenu(menu.id)"><RefreshCw v-if="deletingMenuId===menu.id" class="w-4 h-4 animate-spin" /><Trash2 v-else class="w-4 h-4" /></button>
              </div>
            </div>
          </div>
        </div>
      ...
      <div v-if="activeTab==='category'" class="space-y-3">
        <div class="bg-white border border-slate-100 rounded-3xl p-4 space-y-3">
          <div class="flex items-center justify-between"><h3 class="text-xs font-black uppercase flex items-center gap-1.5"><Tag class="w-4 h-4" /> Kategori Makanan & Minuman</h3><button class="h-8 px-3 rounded-xl bg-slate-900 text-white text-[11px] font-bold flex items-center gap-1" @click="openCategoryModal()"><Plus class="w-3.5 h-3.5" /> Tambah Kategori</button></div>
          <p class="text-[10px] text-slate-500 leading-relaxed">Kelola kategori di panel terpisah agar tidak bercampur dengan menu. Icon kategori crop 1:1 400px.</p>
          <div v-if="categories.length===0" class="py-8 text-center text-slate-400 text-xs">Belum ada kategori. Buat Makanan & Minuman.</div>
          <div v-else class="grid grid-cols-1 gap-2">
            <div v-for="cat in categories" :key="cat.id" class="bg-slate-50 border border-slate-100 rounded-2xl p-3 flex items-center gap-2.5">
              <div class="w-10 h-10 rounded-full bg-white border overflow-hidden flex items-center justify-center shrink-0"><img v-if="cat.image_url" :src="cat.image_url" class="w-full h-full object-cover" /><Tag v-else class="w-4 h-4 text-slate-400" /></div>
              <div class="flex-1 min-w-0"><p class="text-[11px] font-black truncate">{{ cat.name }}</p><p class="text-[9px] text-slate-400">Sort {{ cat.sort_order }} • {{ cat.is_active ? 'Aktif' : 'Nonaktif' }}</p></div>
      ...
      <div v-if="activeTab==='topping'" class="space-y-3">
        <div class="bg-white border border-slate-100 rounded-3xl p-4 space-y-3">
          <div class="flex items-center justify-between"><h3 class="text-xs font-black uppercase flex items-center gap-1.5"><ChefHat class="w-4 h-4" /> Topping Master Independent Shared</h3><button class="h-8 px-3 rounded-xl bg-primary text-white text-[11px] font-bold flex items-center gap-1" @click="openToppingMasterModal()"><Plus class="w-3.5 h-3.5" /> Tambah Topping</button></div>
          <p class="text-[10px] text-slate-500 leading-relaxed">Topping bersifat independent, tidak melekat pada variant. 1 topping bisa dipakai banyak minuman (Keju, Bobba, Eskrim). Kelola sekali di sini.</p>
          <div v-if="toppingMasters.length===0" class="py-8 text-center"><div class="w-12 h-12 mx-auto rounded-2xl bg-amber-50 border border-amber-100 flex items-center justify-center mb-2"><CupSoda class="w-6 h-6 text-amber-400" /></div><p class="text-xs text-slate-400">Belum ada topping master. Buat Keju, Bobba, Eskrim dengan foto 400.</p></div>
          <div v-else class="space-y-2">
            <div v-for="tm in toppingMasters" :key="tm.id" class="bg-amber-50/50 border border-amber-100 rounded-2xl p-3 flex items-center gap-3">
              ...
              <div class="flex-1 min-w-0"><p class="text-xs font-black">{{ tm.name }}</p><p class="text-[10px] text-slate-500">{{ tm.options?.length||0 }} opsi • Shared</p><div class="flex gap-1 mt-1 flex-wrap"><span v-for="opt in (tm.options||[]).slice(0,4)" :key="opt.id" class="px-1.5 py-0.5 rounded-full bg-white border text-[9px] font-bold">{{ opt.label }} +{{ opt.price_delta }}</span></div></div>
              <div class="flex gap-1"><button class="w-7 h-7 rounded-full bg-white border flex items-center justify-center" @click="openToppingMasterModal(tm)"><Edit class="w-3 h-3" /></button><button class="w-7 h-7 rounded-full bg-rose-50 border border-rose-100 text-rose-500 flex items-center justify-center" @click="handleDeleteToppingMaster(tm.id)"><Trash2 class="w-3 h-3" /></button></div>
            </div>
          </div>
        </div>
      </div>
    ...
    <UiModal v-model:open="showAddModal" title="Tambah Menu - Kategori, Varian±Foto">
        ...
        <div class="space-y-2">
          ...
          <div class="flex items-center gap-3">
            ...
            <div class="flex-1 space-y-1">
              <button type="button" class="h-11 px-4 rounded-2xl border bg-slate-50 text-xs font-bold w-full sm:w-auto" @click="()=>addFileInputRef?.click()">{{ previewUrl ? 'Ganti & Crop Otomatis' : 'Pilih → Otomatis Crop 1:1' }}</button>
              ...
              <div v-for="(opt,oIdx) in vg.options" :key="oIdx" class="bg-white border rounded-2xl p-3 space-y-2">
                <div class="flex gap-2">
                  ...
                  <div class="flex-1 space-y-1.5">
                    <input v-model="opt.label" placeholder="Besar" class="w-full h-8 rounded-xl border px-3 text-xs font-bold">
                    <div class="flex gap-2">
                      <input v-model.number="opt.price_delta" type="number" placeholder="+3000/-2000" class="flex-1 h-8 rounded-xl border px-2 text-[11px] font-bold">
                      <button class="h-8 px-3 rounded-xl bg-amber-50 border border-amber-200 text-[10px] font-bold text-amber-700" @click="(()=>{ const input=document.createElement('input'); input.type='file'; input.accept='image/*'; input.onchange=(ev)=>handleFileChange(ev,'variant',{g:gIdx,o:oIdx}); input.click() })()">Foto 600</button>
                    </div>
                  </div>
    ...
    <UiModal v-model:open="showCategoryModal" title="Kategori Makanan/Minuman">
      <div class="space-y-4 p-1">
        ...
        <div class="flex gap-3 pt-3"><button class="flex-1 h-11 rounded-2xl border text-xs font-bold" @click="showCategoryModal=false">Batal</button><button class="flex-1 h-11 rounded-2xl bg-primary text-white text-xs font-bold" :disabled="saveCategoryLoading" @click="handleSaveCategory">{{ saveCategoryLoading ? 'Menyimpan...' : 'Simpan Kategori' }}</button></div>
      </div>
    </UiModal>

    <UiModal v-model:open="showToppingMasterModal" title="Topping Master Independent Shared">
      <div class="space-y-4 p-1 max-h-[70vh] overflow-y-auto">
        <div class="space-y-1.5"><label class="text-[10px] font-bold uppercase">Nama Topping Master</label><input v-model="toppingMasterForm.name" placeholder="Keju, Bobba, Eskrim (shared)" class="h-11 w-full rounded-2xl border px-4 text-xs font-bold"></div>
        <div class="space-y-2">
          <div class="flex items-center justify-between"><p class="text-[11px] font-black uppercase">Opsi Topping dengan Foto (400x400)</p><button class="h-7 px-2.5 rounded-xl bg-primary text-white text-[10px] font-bold" @click="toppingMasterOptions.push({ label:'', price_delta:3000, image_url:'' })">+ Opsi</button></div>
          <div v-for="(opt,idx) in toppingMasterOptions" :key="idx" class="bg-white border rounded-2xl p-3 space-y-2">
            <div class="flex gap-2">
              ...
              <div class="flex-1 space-y-1.5">
                <input v-model="opt.label" placeholder="Eskrim" class="w-full h-8 rounded-xl border px-3 text-xs font-bold">
                <div class="flex gap-2">
                  <input v-model.number="opt.price_delta" type="number" placeholder="3000" class="flex-1 h-8 rounded-xl border px-2 text-[11px] font-bold">
                  <button class="h-8 px-3 rounded-xl bg-amber-50 border border-amber-200 text-[10px] font-bold text-amber-700" @click="(()=>{ const input=document.createElement('input'); input.type='file'; input.accept='image/*'; input.onchange=(ev)=>handleFileChange(ev,'toppingMaster',undefined,undefined,idx); input.click() })()">Foto 400</button>
                </div>
              </div>
        ...
        <div class="flex gap-3 pt-3"><button class="flex-1 h-11 rounded-2xl border text-xs font-bold" @click="showToppingMasterModal=false">Batal</button><button class="flex-1 h-11 rounded-2xl bg-primary text-white text-xs font-bold" @click="handleSaveToppingMaster">Simpan Topping Master</button></div>
      </div>
    </UiModal>

    <CommonImageCropper v-if="cropperOpen" :src="cropperSrc" :type="cropperTarget==='category' ? 'logo' : (cropperTarget==='topping' || cropperTarget==='toppingMaster') ? 'logo' : 'product'" @cropped="onCropped" @cancel="onCropCancel" />
  </div>
</template>

...
// V2 FINAL - 3 Tabs: Menu | Kategori | Tambahan (Add-on)
// Istilah baru: Topping → Tambahan (Bahasa Indonesia lebih cocok untuk UMKM warung, contoh: Keju, Bobba, Eskrim, Sambal, Kerupuk)
// Backend: /merchant/addons (baru) + backward compat /merchant/toppings (alias) - independent shared per merchant
// Kategori: icon foto 1:1 400, sort, active, COS auto-delete
// Tambahan Master: opsi foto 400, price_delta, COS auto-delete, bisa dipakai banyak menu via Pakai cepat
// Menu: kategori, varian ±harga foto 600, tambahan per-menu foto 400, foto produk wajib crop 1:1 1200, COS auto-delete
import { Plus, Edit, Trash2, RefreshCw, Utensils, ArrowLeft, Camera, Tag, Layers, PlusCircle, ChefHat, CupSoda, Sparkles } from '@lucide/vue'
...
const addonMasters = ref<Array<{ id: string; name: string; image_url?: string; sort_order?: number; is_active?: boolean; options: Array<{ id: string; label: string; price_delta: number; image_url?: string; previewUrl?: string }> }>>([])
const showAddonMasterModal = ref(false)
const addonMasterForm = ref({ name: '', image_url: '' })
const addonMasterPreviewUrl = ref('')
...
const fetchProfile = async () => {
  try {
    ...
    if (profile) {
      ...
      // Fetch addon masters - coba endpoint baru /merchant/addons, fallback ke /merchant/toppings alias
      try {
        const { request } = useApi()
        let data: any[] | null = null
        try {
          const res = await request<{ data: any[] }>('/merchant/addons')
          data = res.data
        } catch {
          // fallback alias lama
          try {
            const res2 = await request<{ data: any[] }>('/merchant/toppings')
            data = res2.data
          } catch {}
        }
        if (data && Array.isArray(data)) addonMasters.value = data
        else {
          // Fallback aggregasi dari menu existing topping groups sebagai shared view kalau tabel addon_masters kosong
          if (addonMasters.value.length===0) {
      ...
      } catch (e) { console.warn('[Addons] fetch fallback', e) }
...
const openAddModal = () => {
  ...
  variantGroups.value=[]; addonGroups.value=[]
...
const handleAddMenu = async () => {
  ...
  try {
    ...
    if (menuId) {
      ...
      for (const tg of addonGroups.value) {
    ...
    success('Menu V2 ditambahkan — kategori, varian ±foto, tambahan foto. COS auto-delete saat hapus.')
...
const handleDeleteMenu = async (id:string) => {
  if (!confirm('Yakin hapus menu ini? Gambar produk + varian + tambahan di COS akan dihapus otomatis.')) return
...
// Varian & Tambahan per menu modal
...
// Tambahan Master Shared (istilah baru, sebelumnya Topping Master)
const openAddonMasterModal = (master?:any) => {
  if (master) { addonMasterForm.value={ name:master.name, image_url:master.image_url||'' }; addonMasterPreviewUrl.value=master.image_url||''; addonMasterOptions.value=(master.options||[]).map((o:any)=>({ id:o.id, label:o.label, price_delta:o.price_delta, image_url:o.image_url, previewUrl:o.image_url })); (addonMasterForm as any).id=master.id } else { addonMasterForm.value={ name:'', image_url:'' }; addonMasterPreviewUrl.value=''; addonMasterOptions.value=[{ label:'Keju Parut', price_delta:3000, image_url:'', previewUrl:'' }, { label:'Bobba', price_delta:4000, image_url:'', previewUrl:'' }] }
  showAddonMasterModal.value=true
}
const handleSaveAddonMaster = async () => {
  ...
  try {
    ...
    try {
      ...
      if (id) {
        await request(`/merchant/addons/${id}`, { method:'PUT', body:payload })
      } else {
        await request('/merchant/addons', { method:'POST', body:payload })
      }
      const res=await request<{ data:any[] }>('/merchant/addons')
      if (res.data) addonMasters.value=res.data
    } catch {
      // fallback coba alias lama /merchant/toppings, atau local-only jika backend belum deploy
      try {
        const payload={ name:addonMasterForm.value.name, image_url:addonMasterForm.value.image_url, options: addonMasterOptions.value.map(o=>({ label:o.label, price_delta:o.price_delta, image_url:o.image_url })) }
        ...
        // local-only fallback
        if (id) {
          const idx=addonMasters.value.findIndex(t=>t.id===id)
          if (idx>=0) addonMasters.value[idx]={ id, name:addonMasterForm.value.name, image_url:addonMasterForm.value.image_url, options: addonMasterOptions.value.map((o,i)=>({ id:o.id||`opt-${i}`, label:o.label, price_delta:o.price_delta, image_url:o.image_url||'', previewUrl:o.image_url||'' })) }
        } else {
          addonMasters.value.push({ id:`local-${Date.now()}`, name:addonMasterForm.value.name, image_url:addonMasterForm.value.image_url, options: addonMasterOptions.value.map((o,i)=>({ id:`opt-${i}-${Date.now()}`, label:o.label, price_delta:o.price_delta, image_url:o.image_url||'', previewUrl:o.image_url||'' })) })
        }
      }
    }
    success('Tambahan master disimpan — shared bisa dipakai banyak menu')
    showAddonMasterModal.value=false
  } catch { error('Gagal simpan tambahan master') } finally { addonMasterSaving.value=false }
}
const handleDeleteAddonMaster = async (id:string) => {
  if (!confirm('Hapus tambahan ini? Gambar di COS akan auto-delete.')) return
  try {
    const { request } = useApi()
    try { await request(`/merchant/addons/${id}`, { method:'DELETE' }) } catch { try { await request(`/merchant/toppings/${id}`, { method:'DELETE' }) } catch {} }
    addonMasters.value=addonMasters.value.filter(t=>t.id!==id)
    success('Tambahan dihapus, COS auto-delete')
  } catch { error('Gagal hapus') }
}
const duplicateFromMaster = (master:any) => {
  addonGroups.value.push({ name: master.name, type:'multiple', is_required:false, min_select:0, max_select:null, sort_order:addonGroups.value.length, variant_option_id:null, options: (master.options||[]).map((o:any,i:number)=>({ label:o.label, price_delta:o.price_delta, image_url:o.image_url||'', previewUrl:o.image_url||'', is_available:true, sort_order:i })) })
  ...
  <div class="px-4 pb-24 space-y-4">
    ...
    <div v-else class="space-y-4">
      <div class="flex items-center gap-2.5 pt-1">
        ...
        <div class="flex-1 min-w-0"><h2 class="text-[15px] font-black leading-tight">Katalog Menu V2</h2><p class="text-[10px] text-slate-400 font-medium truncate">Menu · Kategori terpisah · Tambahan shared sampai opsi foto</p></div>
      ...
      <div v-if="activeTab==='menu'" class="space-y-3">
        ...
        <div v-if="filteredMenus.length===0" class="py-10 px-6 text-center bg-white border rounded-[24px]"><div class="w-14 h-14 mx-auto rounded-2xl bg-slate-50 border flex items-center justify-center mb-3"><Utensils class="w-7 h-7 text-slate-400" /></div><p class="text-[13px] font-black">Belum Ada Menu</p><p class="text-[11px] text-slate-500 mt-1 max-w-[260px] mx-auto">Kategori & tambahan sudah dipisah management-nya. Tambah menu dengan foto 1:1, varian foto, tambahan foto.</p><button class="mt-4 h-10 px-5 rounded-full text-xs font-black bg-primary text-white" @click="openAddModal">+ Tambah Menu</button></div>
      ...
      <div v-if="activeTab==='category'" class="space-y-3">
        <div class="bg-white border border-slate-100 rounded-3xl p-4 space-y-3 shadow-sm">
          <div class="flex items-center justify-between"><h3 class="text-xs font-black uppercase flex items-center gap-1.5"><Tag class="w-4 h-4 text-primary" /> Kategori (Terpisah)</h3><button class="h-8 px-3 rounded-xl bg-slate-900 text-white text-[11px] font-bold flex items-center gap-1" @click="openCategoryModal()"><Plus class="w-3.5 h-3.5" /> Tambah</button></div>
          <p class="text-[10px] text-slate-500 leading-relaxed bg-indigo-50/50 border border-indigo-100 rounded-xl p-2.5">Management kategori <b>terpisah</b> dari menu. Setiap kategori punya <b>icon foto 1:1 400x400</b> (opsional), sort_order, is_active. Icon dipakai di filter chips & browse user. COS auto-delete saat kategori dihapus.</p>
      ...
      <!-- TAMBAHAN TAB - Terpisah sampai opsi foto (istilah baru Indonesia) -->
      <div v-if="activeTab==='addon'" class="space-y-3">
        <div class="bg-white border border-slate-100 rounded-3xl p-4 space-y-3 shadow-sm">
          <div class="flex items-center justify-between"><h3 class="text-xs font-black uppercase flex items-center gap-1.5"><Sparkles class="w-4 h-4 text-amber-500" /> Tambahan (Add-on) Master Shared (Terpisah)</h3><button class="h-8 px-3 rounded-xl bg-primary text-white text-[11px] font-bold flex items-center gap-1" @click="openAddonMasterModal()"><Plus class="w-3.5 h-3.5" /> Baru</button></div>
          <p class="text-[10px] text-slate-500 leading-relaxed bg-amber-50/70 border border-amber-100 rounded-xl p-2.5"><b>Tambahan</b> adalah istilah Indonesia yang lebih cocok daripada Topping — untuk UMKM warung seperti <b>Keju, Bobba, Eskrim, Sambal, Kerupuk, Extra Nasi</b>. Bersifat <b>independent shared</b>, 1 master bisa dipakai banyak menu. Setiap opsi punya <b>label, harga tambahan, foto 400x400</b>. Kelola sekali di sini, pakai cepat via "Pakai" saat tambah menu. COS auto-delete.</p>
          <div v-if="addonMasters.length===0" class="py-10 text-center"><div class="w-12 h-12 mx-auto rounded-2xl bg-amber-50 border border-amber-100 flex items-center justify-center mb-2"><CupSoda class="w-6 h-6 text-amber-400" /></div><p class="text-xs text-slate-500">Belum ada tambahan. Buat Keju, Bobba, Eskrim, Sambal, Kerupuk dengan opsi foto 400.</p><button class="mt-3 h-9 px-4 rounded-full bg-primary text-white text-xs font-bold" @click="openAddonMasterModal()">+ Tambah Tambahan</button></div>
    ...
    <UiModal v-model:open="showAddModal" title="Tambah Menu V2 — Kategori, Varian ±Foto, Tambahan">
      <div class="space-y-5 p-1 max-h-[75vh] overflow-y-auto">
        <div class="grid grid-cols-2 gap-3">
          <div class="space-y-1.5 col-span-2"><label class="text-[10px] font-bold uppercase">Nama Menu</label><input v-model="menuForm.name" type="text" placeholder="Ayam Geprek, Es Teler Krimmi..." class="h-11 w-full rounded-2xl border px-4 text-xs font-semibold"></div>
        ...
        <div v-if="addonMasters.length>0" class="space-y-2 border rounded-2xl p-3 bg-amber-50/40 border-amber-100">
          <p class="text-[10px] font-black uppercase flex items-center gap-1"><Sparkles class="w-3 h-3" /> Ambil cepat dari Tambahan Master Shared</p>
          <div class="flex gap-1.5 overflow-x-auto pb-1"><button v-for="tm in addonMasters" :key="tm.id" class="shrink-0 h-7 px-3 rounded-full bg-white border text-[10px] font-bold hover:bg-slate-900 hover:text-white flex items-center gap-1" @click="duplicateFromMaster(tm)"><Plus class="w-3 h-3" />{{ tm.name }}</button></div>
        </div>
        <div class="space-y-3 border-t pt-4">
          ...
          <div v-if="variantGroups.length===0" class="py-3 px-3 rounded-2xl bg-slate-50 border border-dashed text-[10px] text-slate-500 text-center">Belum ada varian. Contoh: Ukuran Regular / Besar +5k dengan foto 600x600.</div>
          <div v-for="(vg,gIdx) in variantGroups" :key="gIdx" class="bg-slate-50 border rounded-2xl p-3 space-y-3">
            <div class="flex gap-2"><input v-model="vg.name" placeholder="Ukuran / Level Pedas" class="flex-1 h-9 rounded-xl border px-3 text-xs font-bold"><select v-model="vg.type" class="h-9 rounded-xl border px-2 text-[10px]"><option value="single">Single (pilih 1)</option><option value="multiple">Multiple</option></select><button class="w-9 h-9 rounded-xl bg-rose-50 border border-rose-100 text-rose-500 flex items-center justify-center shrink-0" @click="removeVariantGroup(gIdx)"><Trash2 class="w-4 h-4" /></button></div>
            ...
            <div class="space-y-2">
              <div v-for="(opt,oIdx) in vg.options" :key="oIdx" class="bg-white border rounded-2xl p-3 space-y-2">
                <div class="flex gap-2">
                  ...
                  <div class="flex-1 space-y-1.5"><input v-model="opt.label" placeholder="Besar / Pedas Lv 3" class="w-full h-8 rounded-xl border px-3 text-xs font-bold"><div class="flex gap-2"><input v-model.number="opt.price_delta" type="number" placeholder="+3000 / -2000" class="flex-1 h-8 rounded-xl border px-2 text-[11px] font-bold"><button class="h-8 px-3 rounded-xl bg-amber-50 border border-amber-200 text-[10px] font-bold text-amber-700" @click="(()=>{ const input=document.createElement('input'); input.type='file'; input.accept='image/*'; input.onchange=(ev)=>handleFileChange(ev as any,'variant',{g:gIdx,o:oIdx}); input.click() })()">Foto 600</button></div></div>
        ...
        <div class="space-y-3 border-t pt-4">
          <div class="flex items-center justify-between"><p class="text-[11px] font-black uppercase flex items-center gap-1"><Sparkles class="w-3.5 h-3.5" /> Tambahan dengan Foto (per menu)</p><button class="h-7 px-2.5 rounded-xl bg-slate-900 text-white text-[10px] font-bold" @click="addAddonGroup">+ Group</button></div>
          <div v-if="addonGroups.length===0" class="py-3 px-3 rounded-2xl bg-amber-50/50 border border-dashed border-amber-200 text-[10px] text-slate-600 text-center">Belum ada tambahan di menu ini. Gunakan cepat dari Tambahan Master shared di atas atau buat baru dengan foto 400.</div>
          <div v-for="(tg,gIdx) in addonGroups" :key="gIdx" class="bg-amber-50/50 border border-amber-100 rounded-2xl p-3 space-y-2">
            <div class="flex gap-2"><input v-model="tg.name" placeholder="Tambahan / Extra" class="flex-1 h-8 rounded-xl border px-2 text-xs font-bold"><button class="w-8 h-8 rounded-xl bg-rose-50 border text-rose-500 flex items-center justify-center" @click="removeAddonGroup(gIdx)"><Trash2 class="w-3.5 h-3.5" /></button></div>
            <div v-for="(opt,oIdx) in tg.options" :key="oIdx" class="bg-white border rounded-xl p-2 flex gap-2 items-center">
              ...
              <input v-model="opt.label" placeholder="Keju / Kerupuk" class="flex-1 h-7 rounded-lg border px-2 text-[11px] font-bold"><input v-model.number="opt.price_delta" type="number" placeholder="+3000" class="w-16 h-7 rounded-lg border px-1 text-[10px] font-bold"><button class="h-7 px-2 rounded-lg bg-amber-50 border text-[9px] font-bold text-amber-700" @click="(()=>{ const input=document.createElement('input'); input.type='file'; input.accept='image/*'; input.onchange=(ev)=>handleFileChange(ev as any,'addon',undefined,{g:gIdx,o:oIdx}); input.click() })()">Foto</button><button class="w-6 h-6 rounded-full bg-rose-50 border text-rose-500 flex items-center justify-center" @click="removeAddonOption(gIdx,oIdx)"><Trash2 class="w-3 h-3" /></button>
            </div>
            <button class="h-7 px-2 rounded-xl bg-slate-900 text-white text-[10px] font-bold" @click="addAddonOption(gIdx)">+ Opsi Tambahan</button>
          </div>
        </div>
    ...
    <UiModal v-model:open="showCategoryModal" title="Kategori — Management Terpisah (Icon Foto)">
      <div class="space-y-4 p-1">
        <div class="bg-indigo-50 border border-indigo-100 rounded-2xl p-2.5 text-[10px] text-indigo-700 leading-relaxed">Kategori dikelola <b>terpisah</b>. Icon foto 1:1 400 opsional, ditampilkan di chips filter & browse user. COS auto-delete saat kategori dihapus.</div>
        <div class="space-y-1.5"><label class="text-[10px] font-bold uppercase">Nama Kategori</label><input v-model="categoryForm.name" placeholder="Makanan, Minuman..." class="h-11 w-full rounded-2xl border px-4 text-xs font-bold"></div>
    ...
    <UiModal v-model:open="showAddonMasterModal" title="Tambahan Master Shared — Sampai Opsi Foto (Istilah Indonesia)">
      <div class="space-y-4 p-1 max-h-[70vh] overflow-y-auto">
        <div class="bg-amber-50 border border-amber-100 rounded-2xl p-2.5 text-[10px] text-amber-800 leading-relaxed"><b>Tambahan</b> (sebelumnya Topping) adalah istilah Indonesia yang lebih cocok untuk warung UMKM — contoh <b>Keju, Bobba, Eskrim, Sambal, Kerupuk, Extra Nasi, Telur</b>. Setiap master punya banyak opsi, setiap opsi punya <b>label + harga tambahan + foto 400x400</b> dengan crop & COS auto-delete. Shared bisa dipakai banyak menu.</div>
        <div class="space-y-1.5"><label class="text-[10px] font-bold uppercase">Nama Tambahan Master (Keju, Sambal, Kerupuk... shared)</label><input v-model="addonMasterForm.name" placeholder="Keju / Sambal / Kerupuk / Extra Nasi (shared)" class="h-11 w-full rounded-2xl border px-4 text-xs font-bold"></div>
        <div class="space-y-2">
          <div class="flex items-center justify-between"><p class="text-[11px] font-black uppercase">Opsi Tambahan dengan Foto 400x400</p><button class="h-7 px-2.5 rounded-xl bg-primary text-white text-[10px] font-bold" @click="addonMasterOptions.push({ label:'', price_delta:3000, image_url:'', previewUrl:'' })">+ Opsi</button></div>
          <div v-for="(opt,idx) in addonMasterOptions" :key="idx" class="bg-white border rounded-2xl p-3 space-y-2">
            <div class="flex gap-2">
              ...
              <div class="flex-1 space-y-1.5"><input v-model="opt.label" placeholder="Keju Mozarella / Sambal Matah / Kerupuk" class="w-full h-8 rounded-xl border px-3 text-xs font-bold"><div class="flex gap-2"><input v-model.number="opt.price_delta" type="number" placeholder="3000" class="flex-1 h-8 rounded-xl border px-2 text-[11px] font-bold"><button class="h-8 px-3 rounded-xl bg-amber-50 border border-amber-200 text-[10px] font-bold text-amber-700" @click="(()=>{ const input=document.createElement('input'); input.type='file'; input.accept='image/*'; input.onchange=(ev)=>handleFileChange(ev as any,'addonMasterOption',undefined,undefined,idx); input.click() })()">Foto 400</button></div></div>
              <button class="w-8 h-8 rounded-xl bg-rose-50 border border-rose-100 text-rose-500 flex items-center justify-center shrink-0" @click="addonMasterOptions.splice(idx,1)"><Trash2 class="w-4 h-4" /></button>
            </div>
          </div>
        </div>
        <div class="flex gap-3 pt-3"><button class="flex-1 h-11 rounded-2xl border text-xs font-bold" @click="showAddonMasterModal=false">Batal</button><button class="flex-1 h-11 rounded-2xl bg-primary text-white text-xs font-bold" :disabled="addonMasterSaving" @click="handleSaveAddonMaster">{{ addonMasterSaving ? 'Menyimpan...' : 'Simpan Tambahan' }}</button></div>
      </div>
    </UiModal>

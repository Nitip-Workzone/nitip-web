...
const qrisFee = computed(() => {
  if (!order.value || order.value.payment_source !== 'qris') return 0
  const accumulated = (order.value.estimated_cost || 0) + order.value.delivery_fee + (order.value.tip_amount || 0)
  const diff = order.value.total_payment - accumulated
    ...
    <div v-if="!loading && order">
      ...
      <div v-if="order.payment_status === 'unpaid' && order.payment_method === 'escrow' && order.payment_source === 'qris' && order.status !== 'cancelled'" class="space-y-6">
        ...
        <div class="bg-white border border-amber-200 rounded-3xl p-6 shadow-sm text-center space-y-5">
          ...
          <div class="max-w-[340px] mx-auto bg-white border border-slate-200 rounded-3xl p-5 shadow-sm flex flex-col items-center space-y-4">
            ...
            <!-- Supported Channels Grid at Bottom - Official brand badges replaces AI PNGs -->
            <div class="w-full border-t border-slate-100 pt-3 flex flex-wrap items-center justify-center gap-1.5">
              <span class="px-2 py-0.5 rounded-full text-[8px] font-black" style="background:#E6F7FC; color:#00AED6; border:1px solid #00AED6">GoPay</span>
              <span class="px-2 py-0.5 rounded-full text-[8px] font-black" style="background:#FEECE8; color:#EE4D2D; border:1px solid #EE4D2D">SPay</span>
              <span class="px-2 py-0.5 rounded-full text-[8px] font-black" style="background:#F0EBFA; color:#4C3497; border:1px solid #4C3497">OVO</span>
              <span class="px-2 py-0.5 rounded-full text-[8px] font-black" style="background:#E7F4FD; color:#118EEA; border:1px solid #118EEA">DANA</span>
              <span class="px-2 py-0.5 rounded-full text-[8px] font-black" style="background:#E6EEF5; color:#003A6E; border:1px solid #003A6E">Mandiri</span>
              <span class="px-2 py-0.5 rounded-full text-[8px] font-black" style="background:#E6F0FA; color:#0060AF; border:1px solid #0060AF">BCA</span>
            </div>
          </div>
      ...
      <div v-if="order && (order.payment_status !== 'unpaid' || order.payment_method !== 'escrow' || order.payment_source !== 'qris' || order.status === 'cancelled')" class="space-y-6">
        ...
        <div class="space-y-3">
          <!-- Bantuan -->
          <button 
            class="w-full bg-white border border-slate-200 hover:bg-slate-50 text-slate-700 text-xs font-bold py-3.5 px-4 rounded-2xl flex items-center justify-center gap-1.5 transition-all"
            @click="router.push(`/support/new?order_id=${order.id}`)"
          >
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8.228 9c.549-1.165 2.03-2 3.772-2 2.21 0 4 1.343 4 3 0 1.4-1.278 2.575-3.006 2.907-.542.104-.994.54-.994 1.093m0 3h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
            Butuh Bantuan? Buat Tiket CS
          </button>
